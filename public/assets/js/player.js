/* Sintoniza web player. Howler.js based; registers a "web" device, then syncs
 * "play" actions to the gPodder API every 15s and on pause/seek/end.
 */
(function () {
    ...
    function ensureDevice() {
        ...
        devicePromise = fetch('/api/2/devices/current/' + DEVICE_ID + '.json', {
            method: 'POST',
...
/* Download button — fetches the media as a blob so the browser saves it
 * with a friendly filename; falls back to a plain navigation on failure.
 */
(function () {
    ...
    var spinner = document.getElementById('btn-download-spinner');
